...
SEV_ORDER = {"高": 3, "中": 2, "低": 1, "high": 3, "medium": 2, "low": 1}
...
def check_required_fields(doc_type: str, text: str, df_required) -> list[Finding]:
    ...
    for _, row in df_required.iterrows():
        row_doc = str(row.get("doc_type", "")).strip()
        if row_doc and row_doc != doc_type:
            continue

        field = str(row.get("field", "")).strip()
        severity = str(row.get("severity", "中")).strip() or "中"
...
def check_conditional_required(doc_type: str, text: str, df_cond) -> list[Finding]:
    """
    条件必填：命中 trigger_any 但未命中 required_any 才提示
...
def check_forbidden_phrases(doc_type: str, text: str, df_forbidden) -> list[Finding]:
    ...
    for _, row in df_forbidden.iterrows():
        ...
        row_doc = str(row.get("doc_type", "")).strip()
        if row_doc and row_doc not in ("全部", "全", "ALL") and row_doc != doc_type:
            continue
        ...
        if phrase in t:
            ...
            findings.append(Finding(
                ...
                title=f"风险用词：{phrase}",
                quote=phrase,
                reason=risk_reason or "用词定性/指责性较强，可能带来责任风险或被退回",
...
def check_closure(doc_type: str, text: str, df_closure) -> list[Finding]:
    ...
    for _, row in df_closure.iterrows():
        ...
        severity = str(row.get("severity", "高")).strip() or "高"
        hint = str(row.get("hint", "")).strip()
...
def check_logic_conflicts(doc_type: str, text: str, df_logic) -> list[Finding]:
    ...
    for _, row in df_logic.iterrows():
        ...
        severity = str(row.get("severity", "低")).strip() or "低"
        hint = str(row.get("hint", "")).strip()
        ...
        if a_hit and b_hit:
            findings.append(Finding(
                ...
                title="可能存在逻辑矛盾/缺项",
                quote=f"{a[0]} & {b[0]}",
                reason="文本同时命中两个触发条件，建议核对是否需要补充说明",
                suggestion=hint or "请核对描述是否一致，必要时补充措施或说明原因"
            ))
...
def run_checks(doc_type: str, text: str, rules: dict[str, Any]) -> dict[str, Any]:
    ...
    df_cond = rules.get("conditional_required")  # 可能不存在
    ...
    out = {
        "doc_type": doc_type,
        "summary": {
            "high": sum(1 for f in findings if f.severity in ("高", "high")),
            "medium": sum(1 for f in findings if f.severity in ("中", "medium")),
            "low": sum(1 for f in findings if f.severity in ("低", "low")),
            "total": len(findings),
        },
        "findings": [
            {
